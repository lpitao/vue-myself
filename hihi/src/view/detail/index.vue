<template>
	<div>
		...
		<div class="content">
			<div class="hot">
				<img @click="$router.openPage()" src="../../assets/img/ces.jpg"/>
			</div>
			<div class="title-box">
				<h3>{{info.title}}</h3>
				<p>{{info.bigContent}}</p>
				<p>{{info.smallContent}}</p>
				<div class="press">
					<span class="money" v-if="info.money">{{info.money}}</span>
					<span class="no-money" v-if="info.noMoney">{{info.noMoney}}</span>
					<span  class="tips" v-if="target in info.tips">{{target}}</span>
				</div>
			</div>
		...
		<!--<div class="detail-footer">
			<div class="left-box">
				...
				<div class="item" @click="$router.openPage('/c')">
					<p>
						<span class="iconfont"></span>
					</p>
					<p class="name">购物车</p>
					<span class="num" v-show="getShopCarLength > 0"></span>
				</div>
			</div>
			<div class="right-box shop-car">
				加入购物车
